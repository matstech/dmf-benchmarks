...
def test_longmemeval_single_question_schedules_qdrant_resources(tmp_path: Path) -> None:
    qdrant_url = os.getenv("DMF_BENCH_TEST_QDRANT_URL")
    if not qdrant_url:
        pytest.skip("Set DMF_BENCH_TEST_QDRANT_URL to run Qdrant integration.")

    dataset_path = Path(__file__).parents[1] / "fixtures" / "longmemeval-mini.json"
    config = {
        ...
        "selection": {"ordered_item_ids": ["lme-001"], "filters": {}, "seed": 7},
        "models": {
            "answerer": {
                "provider": "fake",
    ...
    unit = LongMemEvalAdapter().enumerate_units(config)[0]
