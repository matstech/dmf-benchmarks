"""Pure LongMemEval retrieval-stage ablation evaluator for v2 outputs."""
...
def evaluate_ablation(evaluations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in evaluations:
        ...
        stats["raw_unmappable_candidates"] += len(raw_candidates) - len(
            [
                candidate
                for candidate in raw_candidates
                if record_id_mapping.get(
                    str((candidate.get("record") or {}).get("record_id", ""))
                )
            ]
        )
        raw_metrics = (
